	...
	<div class="account-address">
		<div class="gray-box clear">
			<div class="title pre-title">
				...
				<span class="gray-blue-btn js-add-address" @click="show"><a class="add">添加新地址</a></span>
			</div>
			<div class="box-inner clear">
				<div class="address-list-item default-item" data-id="765532" v-for="(time,index) in dizhi">
					<div class="name fn-left">
						<div class="name-cell">{{ time.name }}</div>
					</div>
					<div class="detail fn-left">
						<div class="detail-cell">{{ time. province}} {{ time.city }} {{ time.county }} {{ time.add }}</div>
					</div>
					<div class="operation fn-right">
						<div class="operation-cell">
							<a class="gray-edit-btn js-edit-address" @click="updata(time, index)">修改</a>
							<span class="red-del-btn js-del-address" @click="delzhi(index)"> <a>删除</a><em>删除</em> </span>
						</div>
					</div>
					<div class="default fn-right" @click="moren(index)"> 
						<span v-if="time.default">（默认地址)</span>
						<span v-else>(设置默认)</span>
					  </div>
					<div class="telephone fn-right">{{ time.phone }}</div>
				</div>
			<!--tian  -->
			<dizhi v-if="dishow" @dishow="show" :ubdata = "ubdata"></dizhi>
			</div>
		</div>
	</div>
</div>
...
export default {
	...
	methods: {
		delzhi (id) {
			this.$store.commit('delzhi',id)
		},
		show () {
			this.dishow = !this.dishow
		},
		moren (id) {
