    ...
    <div class="content page-order-payment">
        <div class="gray-box clear">
            ...
            <div class="box-inner payment-checkout-panel clear">
                <span class="jianguo-blue-main-btn big-main-btn js-payment-order" v-if="!data.fukuan" @click="zhifu(data.timeid)">
                    <a>现在支付</a>
                </span>
                <span v-else class="is-pay">已支付成功</span>
                <span class="prices"> 应付金额：   <em><span>¥ </span>{{ data.zongjia+data.yunfei }}</em>   </span>
            </div>
        </div>
        <div class="confirm-detail">
            <div class="info-title">订单编号</div>
            <p class="info-detail">{{ data.timeid }}</p>
        </div>
        <div class="confirm-detail">
            <div class="info-title">收货信息</div>
            <p class="info-detail">姓名：{{ data.dizhi.name }}</p>
            <p class="info-detail">联系电话：{{ data.dizhi.phone }}</p>  <p class="info-detail">详细地址：{{ data.dizhi.add }}</p>
        </div>
        <div class="confirm-detail">
            <div class="info-title">发票信息</div>
            <p class="info-detail">发票类型：电子发票</p>
            <p class="info-detail">发票抬头：{{ data.fapiao }}</p>
        ...
        <div class="confirm-goods-table">
            <div class="cart-items clear" v-for="(time,index) in data.shanpin">
                <div class="name fn-left hide-row">
                    <div class="name-cell"> 
                        <a href="http://www.smartisan.com/shop/#/item/100029901" title="Smartisan 快充移动电源 10000mAh（灰色）" target="_blank">{{ time.title }}（{{ time.spec_json.show_name }}）</a><br>
                    </div>
                </div>
                <div class="subtotal fn-right">
                    <div class="subtotal-cell">¥ {{ time.price*time.count }}</div>
                </div>
                <div class="goods-num fn-right">{{ time.count }}</div>
                <div class="price fn-right">¥ {{ time.price }}</div> 
            </div>
        </div>
        <div class="order-discount-line">
            <p> 商品总计： <span>¥ {{ data.zongjia }}</span> </p>
            <p> 运费： <span>+ ¥{{ data.yunfei }}</span> </p>
        </div>
    </div>
</div>  
</template>
...
export default {
    computed: {
        data () {
            // console.log('dgg')
            let data = this.$store.state.dingdan.filter(time =>{
                return time.timeid === this.$route.query.datid
            })[0]
            return data
        }
    },
    methods: {
        zhifu (id) {
            this.$store.commit('zhifu', id)
            alert('确认支付'+(this.data.zongjia+this.data.yunfei)+'元')
