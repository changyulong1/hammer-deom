		...
		<div class="cart-box">
			...
			<div class="cart-inner">
				<div class="empty-label " v-if="count <=0">
					<h3>您的购物车中还没有商品</h3>
					<router-link to="/parts" class="link">现在选购</router-link>
				</div>
				<div v-else>
					...
					<div class="cart-table" v-for="(time,index) in timedata">
						<div class="cart-group">
							<!--购物列表-->
							<div class="cart-top-items">
								<div class="cart-items">
									<div class="items-choose">
										<span class="blue-checkbox-new" :class="{'checkbox-on': time.chek}" @click="active(time.sku_id)"><a></a></span>
									</div>
									...
									<div class="name hide-row">
										<div class="name-table">
											<a href="javascript:;" target="_blank">{{ time.title }}(后壳用）</a>
											<ul class="attribute">
												<li>{{ time.spec_json.show_name }}</li>
											</ul>
										</div>
									</div>
									<div class="operation">
										<a class="items-delete-btn" @click="deldata(time.sku_id)"></a>
									</div>
									<div class="subtotal">¥ {{ time.price*time.count }}</div>
									<div class="item-cols-num">
										<div class="select js-select-quantity">
											<span class="down" :class="{'down-disabled': time.count <=1}" @click="countjian(time.sku_id)">-</span>
											<span class="num">
												<!-- <input type="text" style="display: inline-block;"> -->
												{{ time.count }}
											</span>
											<span class="up" @click="countjia(time.sku_id)">+</span>
				...
				<div class="cart-bar-operation">
					<div>
						<div class="choose-all js-choose-all">
							<span  class="blue-checkbox-new" :class="{'checkbox-on': quanxuan}" @click="allactive(quanxuan)"><a></a></span>
							全选
						</div>
						<div class="delete-choose-goods" @click="delchek">删除选中的商品</div>
					</div>
				</div>
				<div class="shipping">
					...
					<span :class="{'disabled-btn': zonglag <=0}" class="jianguo-blue-main-btn big-main-btn js-checkout"><router-link to="/jiesuan">现在结算</router-link></span>
				</div>
			</div>
		</div>
	</div>
...
export default {
    ...
    quanxuan () {
      return this.$store.getters.quanxuan
    },//选择的商品个数
    zonglag () {
      return this.$store.getters.zonglag
    },//选择商品的总价
    zongjian () {
	...
	methods: {
		...
		countjia (id) {
			this.$store.commit('countjia',id)
		},
		countjian (id) {
			this.$store.commit('countjian',id)
		},
    active (id) {
    ...
    allactive (xuan) {
      console.log(xuan)
      this.$store.commit('allactive',xuan)
    },
    delchek () {
      this.$store.commit('delchek')
    }
	}
};
</script>
