...
<div id="main">
		...
		<div class="item-box">
			<div class="gallery-wrapper">
				<div class="gallery">
					<div class="thumbnail">
						<ul>
							 <li @click="active(index)" :class="{'on': timeshow == index}" v-for="(time,index) in time.ali_images">
                                <img :src="time+'?x-oss-process=image/resize,w_45/quality,Q_80/format,webp'">
                            </li>
						</ul>
					</div>
					<div class="thumb">
						<ul>
							<li :class="{'on': timeshow == index}" v-for="(time,index) in time.ali_images">
                                <img :src="time+'?x-oss-process=image/resize,w_450/quality,Q_80/format,webp'">
                            </li>
			...
			<div class="banner">
				<div class="sku-custom-title">
					<div class="params-price">
						<span><em>¥</em><i>{{ time.price }}</i></span>
					</div>
					<div class="params-info">
						<h4> {{ time.title }}</h4>
						<h6>{{ time.sub_title }}</h6>
					</div>
				</div>
				<div class="sku-dynamic-params-panel">
					<div class="sku-dynamic-params clear">
						...
						<ul class="params-colors">
							<li class="cur" v-for="(time,index) in time.sku_list">
								<router-link :to="{name: 'xiang',query: {timeid: time.id}}"><img :src="'http://img01.smartisanos.cn/'+time.image+'20X20.jpg'"></router-link>
							</li>
						</ul>
					</div>
					<div class="sku-dynamic-params clear">
						...
						<div class="params-detail clear">
							<div class="item-num js-select-quantity">
								<span class="down" :class="{'down-disabled':count<=1}" @click="jian">-</span>
								<span class="num">{{ count }}</span>
								<span class="up" :class="{'up-disabled':count>= time.limit_num}" @click="zeng">+</span>
							</div>
						</div>
					</div>
				</div>
				<div class="sku-status">
					<div class="cart-operation-wrapper clearfix">
						<span class="blue-title-btn js-add-cart" @click="goushangpin" ><a>加入购物车</a></span>
    ...
    <chuang v-if="show"></chuang>
</div>
</template>
<script>
import timedata from '../lib/newItemsData.js'
...
export default {
    ...
    computed: {
        time () {
            let indexdata = this.timedata.filter(time => {
                return Number(time.sku_id) == Number(this.timeid)
            })[0]
            return indexdata
        },
        show () {
    ...
    methods: {
        ...
        goushangpin () {
            let iofodata = {idata: this.time, count: this.count}
            this.$store.commit('gouwudata', iofodata)
        },
        zeng () {
            this.count++
            if(this.count > this.time.limit_num) {
                this.count = this.time.limit_num
            }
        },//减少
        jian () {
            this.count--
