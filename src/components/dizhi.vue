  ...
  <div class="module-dialog clear module-dialog-address module-dialog-show">
    <div class="dialog-panel">
        <div class="topbar">
            ...
            <span class="dialog-close" @click="hieedn">x</span>
        </div>
                        ...
                        <div class="address-form">
                            <div class="module-form-row">
                                <div class="form-item-v3">
                                    <i v-show="!dizhi.name">收货人姓名</i>
                                    <input type="text" class="js-verify" v-model="dizhi.name">
                                    <div class="verify-error"></div>
                                </div>
                            </div>
                            <div class="module-form-row">
                                <div :class="{'form-invalid-item':youwu}" class="form-item-v3" >
                                    <i v-show="!dizhi.phone">手机号</i>
                                    <input type="text" class="js-verify" v-model="dizhi.phone" @blur="setblur" @focus="setfocus">
                                    <div class="verify-error" v-show="youwu">手机号有误</div>
                                </div>
                            </div>
                            <div class="module-form-row clear">
                                <div class="form-item-v3 area-code-w fn-left form-valid-item">
                                    <i v-show="!dizhi.areaCode">区号（可选）</i>
                                    <input type="text" class="js-verify js-address-area-code" v-model="dizhi.areaCode">
                                    <div class="verify-error"></div>
                                </div>
                                <div class="form-item-v3 telephone-w fn-right form-valid-item">
                                    <i v-show="!dizhi.landLine">固定电话（可选）</i>
                                    <input type="text" class="js-verify js-address-telephone" v-model="dizhi.landLine">
                                    <div class="verify-error"></div>
                                </div>
                            </div>
                            <div class="module-form-row clear">
                                <div class="form-item-v3 select-item province-wrapper">
                                    <select name="province_code" class="province select-province js-form-province js-verify" v-model="dizhi.provinceId">
                                        <option value="0">请选择省份</option>
                                        <option v-for="(time,index) in addlist" :value="time.area_id">{{time.area_name}}</option>
                            ...
                            <div class="module-form-row clear">
                                <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                    <select class="city select-city js-form-city js-verify" v-model="dizhi.cityId">
                                        <option value="0">请选择城市</option>
                                         <option v-for="(time,index) in chnegshi" :value="time.area_id">{{time.area_name}}</option>
                                    </select>
                                </div>
                                <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                    <select class="city select-city js-form-city js-verify" v-model="dizhi.countyId">
                                        <option value="0">请选择区县</option>
                                         <option v-for="(time,index) in xianji" :value="time.area_id">{{time.area_name}}</option>
                                    </select>
                                </div>
                            </div>
                            <div class="module-form-row">
                                <div class="form-item-v3">
                                    <i v-show="!dizhi.add">详细地址，如街道名称，楼层，门牌号码等</i>
                                    <input type="text" class="js-verify" v-model="dizhi.add">
                                    <div class="verify-error"></div>
                                </div>
                            </div>
                            <div class="module-form-row fn-clear">
                                <input type="checkbox" class="hide">
                                <span :class="{'blue-checkbox':dizhi.default}" @click="dianjia" class="blue-checkbox-on" ></span>设为默认
                            </div>
                            <div  :class="{'disabled-btn': !baocun}" class="dialog-blue-btn big-main-btn js-verify-address" @click="tijiao">
                                <a>保存</a>
                            </div>
                        </div>
                    </div>
...
export default {
	...
	methods: {
		...
		dianjia () {
			this.dizhi.default = !this.dizhi.default
		},
		cun () {
			if(this.dizhi.name && this.dizhi.phone && this.dizhi.provinceId && this.dizhi.cityId && this.dizhi.add ) {
	          this.baocun = true
	        }else{
	          this.baocun = false
	        }
		...
		tijiao () {
			console.log(555)
			if(this.baocun) {
				this.$store.commit('tijiao',this.dizhi)
				this.hieedn()
			}
		}
	},
	watch: {
		'dizhi.countyId':function () {
			this.xianji.forEach(time => {
				if(time.area_id == this.dizhi.countyId) {
					this.dizhi.county = time.area_name
					return
				}
			})
		},//深度监控
	...
	computed: {
		chnegshi () {
			let cheng = []
			this.addlist.forEach(time => {
				if(time.area_id == this.dizhi.provinceId) {
					this.dizhi.province = time.area_name
					cheng = time.city_list
					return
				}
			})
			if(this.dizhi.provinceId == "0") {
				this.dizhi.provinceId =0
				this.dizhi.cityId =0
			}
			return cheng
		},
		xianji () {
			let xian = []
			this.chnegshi.forEach(time => {
				if(time.area_id == this.dizhi.cityId) {
					this.dizhi.city = time.area_name
					xian = time.county_list
					return
				}
			})
			if(this.dizhi.provinceId == "0") {
