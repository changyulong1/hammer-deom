	...
	<div class="content page-order-checkout checkout">
				...
				<div class="box-inner js-checkout-address-panel ">
					<div class="address-common-table js-multiple-address-panel">
						<ul class="address-item-list clear js-address-item-list">
							<li class="js-choose-address" @click="dataclick(index)" :class="{'selected-address-item':indexdata == index}" v-for="(time,index) in dizhi">
								<div class="address-item">
									<div class="name-section">{{ time.name }}</div>
									<div class="mobile-section">{{ time.phone }}</div>
									<div class="detail-section"> {{ time.province }} {{ time.city }} {{ time.county }}<br> {{ time.add }} </div>
								</div>
							...
							<li class="add-address-item js-add-address" @click="show">
								<p >使用新地址</p>
							</li>
						</ul>
					</div>
				...
				<div class="invoice-detail"> 发票抬头： 
					<div class="radio-box"> 
						<label> 
							...
							<span :class="{'blue-radio-on': !fapiao.show}" @click="activehidd(false)" class="blue-radio "><a></a></span>  个人
						</label> 
						<label> 
							<input type="radio" class="hide"> 
							<span :class="{'blue-radio-on':  fapiao.show}"  @click="activehidd(true)" class="blue-radio" ><a></a></span>  单位
						</label> 
					</div> 
					<div class="module-form-row form-item fn-hide js-invoice-title" v-if="fapiao.show"> 
						<div class="module-form-item-wrapper no-icon small-item"> 
							<i v-show="!fapiao.name">请填写公司发票抬头</i> 
							<input type="text" v-model="fapiao.name" class="js-verify">
              <div v-show="!fapiao.name" class="verify-error">必填</div> 
						</div> 
					</div> 
				</div> 
		...
		<div class="gray-box">
			...
			<div class="box-inner ui-goods-cart">
				...
				<div class="cart-table" v-for="(time,index) in chekdata">
					<div class="cart-group js-cart-group">
						<div class="cart-items">
							<div class="items-thumb">  
								<a href="javascript:;" target="_blank"><img :src="time.ali_image+'?x-oss-process=image/resize,w_80/quality,Q_100/format,webp'"></a>  
							</div>
							<div class="name hide-row">
								<div class="name-cell"> 
									<a href="javascript:;" :title="time.title +'(后壳用)（'+time.spec_json.show_name+'）'" target="_blank">{{ time.title }} (后壳用)（{{ time.spec_json.show_name }}）</a>
								</div>
							</div>
							<div class="subtotal">
								<div class="subtotal-cell">  ¥ {{ time.price*time.count }}  </div> 
							</div>
							<div class="goods-num">{{ time.count }}</div>
			...
			<div class="box-inner"> 
				<div class="order-discount-line"> 
					<p> 商品总计： <span>¥ {{ zongjian }}</span> </p> 
					<p> 运费： <span>+ ¥ {{ yunfei }}</span> </p>  
					<p class="discount-line js-discount-cash"> <em>现金券</em>： <span> - 0 </span> </p>  
				</div> 
			</div>
			<div class="box-inner"> 
				<div class="last-payment clear"> 
					<span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="dingdan"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{ zongjian+yunfei }}</em></span> 
				</div> 
			</div>
		</div>
	</div>
   <dizhi v-if="dishow" @dishow="showdi"></dizhi>
</div>
</template>
<script>
...
export default {
    ...
    created () {
        this.$store.state.receiveInfo.forEach((time,index) => {
               if(time.default) {
    ...
    computed:{
      chekdata () {
        return this.$store.getters.chekdata
      },
      zongjian () {
        return this.$store.getters.zongjian
      },
      yunfei () {
        let fei = 8.00
        if(this.zongjian >= 80) {
    ...
    methods: {
      ...
      activehidd(blan) {
          this.fapiao.show = blan
      },
      dingdan () {
        if(this.fapiao.show && !this.fapiao.name ) return
          let dizhi = this.dizhi[this.indexdata]
        if(!this.fapiao.show) {
          this.fapiao.name = '个人'
        }
        let datatime = new Date()
        let month = datatime.getMonth()+1
        let data = datatime.getDate()
        if(month <=1 && month<=9 ){
          month = '0'+month
        }
        if(data>=1&&data<=9) {
        ...
        let timedata = {
            timeid: datatime.getTime(),
            shanpin: this.chekdata,
            zongjia: this.zongjian,
            yunfei: this.yunfei,
            fapiao: this.fapiao.name,
            date: datatime.getFullYear()+"-"+month+"-"+data,
            dizhi: dizhi,
            fukuan: false
        } 
        this.$store.commit('dingdan', timedata)
        this.$router.push({name:'tijiao',query: {datid: timedata.timeid}})
      }
    },
    components: {
      dizhi
